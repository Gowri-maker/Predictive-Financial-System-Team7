...
<style>
    ...
    li[role="option"]:hover { background-color: rgba(201,123,75,0.12) !important; }
    label { color: #a07850 !important; }
</style>
...
if st.button("🤖 Check Feasibility with AI", use_container_width=True):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # -------- MODEL IS THE SOLE DECISION MAKER --------
            final_prediction = result["prediction"]

            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown(section_title("📋 AI Decision"), unsafe_allow_html=True)
            ...
            if final_prediction == "Feasible":
                ...
                <div style="background:rgba(72,187,120,0.1);border:1px solid rgba(72,187,120,0.35);
                    ...
                    <div>
                        <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:#48bb78;">
                            Feasible — Go ahead!</div>
                        <div style="color:#64748b;font-size:13px;margin-top:4px;">
                            The AI model approves this purchase based on your financial profile.</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                ...
                <div style="background:rgba(252,129,129,0.1);border:1px solid rgba(252,129,129,0.35);
                    ...
                    <span style="font-size:40px;">🔴</span>
                    <div>
                        <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:#fc8181;">
                            Not Feasible — Avoid this purchase</div>
                        <div style="color:#64748b;font-size:13px;margin-top:4px;">
                            The AI model advises against this purchase based on your financial profile.</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with qa2:
                after  = remaining_budget - purchase_amount
                a_col  = "#48bb78" if after >= 0 else "#fc8181"
                a_bg   = "rgba(72,187,120,0.08)" if after >= 0 else "rgba(252,129,129,0.08)"
                st.markdown(metric_card("Budget After Purchase", f"₹{after:,.2f}",                       "💳", a_col, a_bg), unsafe_allow_html=True)
            with qa3:
                p_col  = "#48bb78" if final_prediction == "Feasible" else "#fc8181"
                p_bg   = "rgba(72,187,120,0.08)" if final_prediction == "Feasible" else "rgba(252,129,129,0.08)"
                st.markdown(metric_card("Purchase Amount",      f"₹{purchase_amount:,.2f}",               "🛒", p_col, p_bg), unsafe_allow_html=True)

            # -------- SHAP EXPLANATION --------
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown(section_title("🧠 AI Explanation (SHAP)"), unsafe_allow_html=True)

            for line in result["explanation"]:
                st.markdown(f"""
                <div style="background:rgba(201,123,75,0.06);border:1px solid rgba(201,123,75,0.2);
                    border-left:3px solid #c97b4b;border-radius:8px;padding:12px 16px;
                    margin-bottom:8px;font-size:13px;color:#3d2b1f;">💡 {line}</div>
                """, unsafe_allow_html=True)
            ...
            if final_prediction == "Feasible" and result.get("best_deals"):
                ...
                <div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
                    <div style="background:rgba(72,187,120,0.1);border:1px solid rgba(72,187,120,0.3);
                        ...
                        🔍 {product_name}</div>
                    <div style="background:rgba(201,123,75,0.1);border:1px solid rgba(201,123,75,0.3);
                        border-radius:20px;padding:6px 16px;font-size:13px;font-weight:600;color:#c97b4b;">
                        💰 Filtered under ₹{int(purchase_amount):,}</div>
                </div>
                """, unsafe_allow_html=True)

                platform_icons  = {"Amazon": "🛒", "Flipkart": "🏷️", "Myntra": "👗", "Nykaa": "💄", "Meesho": "🛍️"}
                platform_colors = {"Amazon": "#FF9900", "Flipkart": "#2874F0", "Myntra": "#FF3F6C", "Nykaa": "#FC2779", "Meesho": "#9B59B6"}
                ...
                for i, deal in enumerate(result["best_deals"]):
                    ...
                    with cols[i]:
                        ...
                        <div style="background:#fff8f2;border:1px solid rgba(201,123,75,0.2);
                            border-top:3px solid {color};border-radius:12px;padding:20px 16px;
                            text-align:center;cursor:pointer;">
                            <div style="font-size:28px;margin-bottom:8px;">{icon}</div>
                            <div style="font-weight:600;font-size:14px;color:#3d2b1f;margin-bottom:4px;">{platform}</div>
                            <div style="font-size:11px;color:#c97b4b;font-weight:600;">Under ₹{int(purchase_amount):,} →</div>
                        </div></a>
                        """, unsafe_allow_html=True)
        ...
        <div style="background:rgba(246,173,85,0.1);border:1px solid rgba(246,173,85,0.3);
            ...
            ⚠️ Could not connect to AI backend.<br>
            <small>Run: <code>uvicorn backend.main:app --reload</code></small>
        </div>
        """, unsafe_allow_html=True)
    except requests.exceptions.Timeout:
        st.warning("⚠️ AI backend timed out. Please try again.")